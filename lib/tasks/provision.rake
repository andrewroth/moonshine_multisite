...
To download all the database info, run:
      ...
      cd /var/www/common.local/current
...
def provision(server, server_config, utopian, apps_filter = nil)
  ...
  for app, repo in multisite_config_hash[:apps]
    ...
    multisite_config_hash[:stages].each do |stage|
      ...
      eb_file_path = "app/manifests/assets/private/eventbrite/eventbright.#{utopian_name}.rb"
      if File.exists?(eb_file_path)
        @cap_config.put(File.read(eb_file_path), "#{@cap_config.fetch(:shared_path)}/config/initializers/eventbright.rb")
      end
